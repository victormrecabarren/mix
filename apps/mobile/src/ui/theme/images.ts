// Image helpers. Actual assets live on the active theme so changing
// `theme/index.ts` changes the visual system in one place.
...
export function imageForKey(key: string | undefined): string | null {
  if (!key) return null;
